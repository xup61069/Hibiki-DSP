...
- 會新增/刪除 tracked 檔案的切片，一律把 `docs/state/BASELINE.md` 放進 `shared_paths`，
  並機械式更新摘要裡的 tracked paths 與 repository JSON 兩個數字（#21 門檻會 fail-closed；
  #51 之後 PR CI 對「未動 BASELINE」的 PR 容忍此漂移，但本機 strict 執行仍需更新）。
  數字以 `tools/docs-check.ps1` 的錯誤訊息為準；衝突合併後若摘要句出現重複行，負責去重。
