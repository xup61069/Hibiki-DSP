...
if ($SelfTest) {
  ...
  $sourceFixture = @{
    ...
    offscreen = "let capturing = false; let bridge = null; let bridgeRetryTimer = null; let bridgeRetryAttempt = 0; const BRIDGE_RETRY_MAX_ATTEMPTS = 10; function connectBridge() { if (!capturing || bridge) return; try { bridge = new WebSocket('ws://127.0.0.1:17842/v1/tab'); bridge.binaryType = 'arraybuffer'; bridge.onopen = () => {}; bridge.onclose = () => { bridge = null; scheduleBridgeRetry(); }; } catch (_) { bridge = null; scheduleBridgeRetry(); } } function scheduleBridgeRetry() { if (!capturing || bridgeRetryTimer !== null || bridgeRetryAttempt >= BRIDGE_RETRY_MAX_ATTEMPTS) return; bridgeRetryAttempt++; bridgeRetryTimer = setTimeout(() => { bridgeRetryTimer = null; connectBridge(); }, 1000); } function cancelBridgeRetry() { if (bridgeRetryTimer !== null) { clearTimeout(bridgeRetryTimer); bridgeRetryTimer = null; } bridgeRetryAttempt = 0; } chrome.runtime.onMessage.addListener(async (message, _sender, sendResponse) => { if (message.type === 'stop-tab-stream') { capturing = false; cancelBridgeRetry(); activeStream.getTracks().forEach(track => track.stop()); sendResponse({stopped: true}); return true; } if (message.type === 'get-capture-state') { sendResponse({capturing, bridgeConnected: false}); return false; } if (message.type !== 'start-tab-stream') return false; const context = new AudioContext(); await context.audioWorklet.addModule('audio-worklet.js'); const node = new AudioWorkletNode(context, 'hibiki-tab-packetizer'); const constraints = {audio: {mandatory: {chromeMediaSource: 'tab', chromeMediaSourceId: message.streamId}}, video: false}; await navigator.mediaDevices.getUserMedia(constraints); const stream = await navigator.mediaDevices.getUserMedia(constraints); activeStream = stream; track.addEventListener('ended', handleSourceEnded); sendResponse({ok: true}); capturing = true; connectBridge(); return true; }); async function handleSourceEnded() { capturing = false; cancelBridgeRetry(); await teardownCaptureGraph(); reportState(); chrome.runtime.sendMessage({type: 'offscreen-capture-released'}); }"
