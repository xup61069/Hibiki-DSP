...
function Assert-ExtensionSourcePolicy(
  ...
  if ($popupSource -notmatch 'bridgeReconnectState') {
    throw ("Popup must consume bridge reconnect progress state in {0}." -f $sourceName)
  }
  ...
  foreach ($progressPattern in @(
      ...
      'BRIDGE_RECONNECT_WAITING_V1',
      'BRIDGE_RECONNECT_RETRYING_V1',
      'BRIDGE_RECONNECT_EXHAUSTED_V1',
      'bridgeReconnectState\s*:\s*bridgeReconnectState\s*\(\s*\)'
    )) {
    if ($offscreenSource -notmatch $progressPattern) {
...
if ($SelfTest) {
  ...
  $sourceFixture = @{
    popup = "button.addEventListener('click', async () => { delete status.dataset.error; const response = await chrome.runtime.sendMessage({type: 'capture-active-tab', tabId: tab.id}); if (response?.ok) { render(); } else { status.textContent = response?.error ?? 'Capture failed'; status.dataset.error = 'true'; render(); } }); stopButton.addEventListener('click', async () => { delete status.dataset.error; const response = await chrome.runtime.sendMessage({type: 'stop-capture'}); if (response?.ok) { applyState(); } else { status.textContent = response?.error ?? 'Stop failed'; status.dataset.error = 'true'; } }); chrome.runtime.onMessage.addListener((message) => { if (message.type === 'capture-state') applyState(message); }); document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') { void refreshState(); } }); refreshState(); async function refreshState() { await chrome.runtime.sendMessage({type: 'get-capture-state'}); } function applyState(state) { bridgeReconnectState = state?.bridgeReconnectState ?? idle; render(); }"
  ...
  $missingReconnectStateSource = $sourceFixture.offscreen -replace 'bridgeReconnectState', 'removedProgress'
  $caught = $false
  try { Assert-ExtensionSourcePolicy $sourceFixture.popup $sourceFixture.serviceWorker $missingReconnectStateSource $sourceFixture.worklet 'selftest-missing-reconnect-state' } catch { $caught = $true }
  if (-not $caught) { throw 'SelfTest expected missing reconnect state failure.' }
