...
if (-not $IsWindows) { throw 'Live process-loopback probe is Windows-only.' }
$build = Join-Path $repo '.local/live-process-loopback-build'
New-Item -ItemType Directory -Path $build -Force | Out-Null
cmake -S $repo -B $build -DHIBIKI_BUILD_TESTS=ON -DHIBIKI_BUILD_LIVE_PROBES=ON
if ($LASTEXITCODE -ne 0) { throw "CMake configure failed with exit code $LASTEXITCODE" }
cmake --build $build --config RelWithDebInfo --target hibiki_live_process_loopback_probe --parallel
if ($LASTEXITCODE -ne 0) { throw "Probe build failed with exit code $LASTEXITCODE" }
$probe = Join-Path $build 'tests/RelWithDebInfo/hibiki_live_process_loopback_probe.exe'
