...
- 第二十二波整合增量：noise gate 修正開關方向，讓開啟使用 attack、關閉使用 release（Issue #636 / PR #640）；共用 handoff 稽核新增同 branch 拒絕的離線自檢（Issue #643 / PR #644）；Engine Preview 在同步建立 canonical pipe 後立即 stop 時可馬上取消 pending connect，不再等完整 idle timeout（Issue #637 / PR #645）。皆為 user-space/source evidence；不宣稱實體音訊或 driver 載入。
