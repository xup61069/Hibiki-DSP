...
- 第三十八波整合增量：自訂 Scene catalog 離線同步佇列加入持久化，重新啟動後依序回放待送 Upsert/Remove，原子替換保留 64 筆上限與累計捨棄警告，malformed/oversized fail-closed 且不覆蓋已載入卡片，佇列保存失敗會回復卡片變更（Issue #855 / PR #866）；新增 scene-sync-queue-v1 JSON Schema，並以 if/then/else 強制 upsert 需帶 name/output_group、remove 兩者必須為空字串（Issue #878 / PR #879、Issue #884 / PR #885）。公開 schema 全面收緊：session-route-rules $id URI 正規化、catalog constraints 加嚴、driver-control message-type 精確欄位集合、scene identities/route matchers 拒絕空白、rule_id pattern+bounds、custom scene card display fields 字串型別（Issue #886 / PR #892、Issue #889 / PR #896、Issue #893 / PR #900、Issue #888 / PR #902、Issue #907 / PR #908、Issue #909 / PR #911、Issue #910 / PR #913）。PowerShell gates 全面啟用 strict-mode guards 並補上漏網腳本；docs-check SelfTest 修復 early-exit、doctor.ps1 strict registry/count 修復、所有 tracked schemas 成為必要入口並驗證結構與唯一 $id；installer ReleaseManifest validation 啟用 strict mode 且要求 product_version（Issue #864 / PR #867、Issue #870 / PR #876、Issue #881 / PR #882、Issue #887 / PR #898、Issue #891 / PR #897、Issue #912 / PR #915、Issue #914 / PR #916）。evidence 與文件完整性：legacy evidence JSON 補滿 source_commit（73 檔、66 處編輯），兩份舊式 evidence 移除從未存在的 $schema 引用，README/PROJECT_MAP driver 狀態同步至隔離 guest PnP-start evidence，SPEC-0008 對齊 lane latency wiring 已接線的事實（Issue #904 / PR #906、Issue #917 / PR #918、Issue #890 / PR #899、Issue #863 / PR #865）。上述分別為 control-model persistence evidence、schema/tooling/installer gate evidence 與 documentation/evidence-metadata correction；不宣稱瀏覽器 runtime automation、實體音訊量測、driver 實體播放／HLK／Microsoft signing。
