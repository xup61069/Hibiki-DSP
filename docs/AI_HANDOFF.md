...
- 第三十五波整合增量：DesktopCompat 與 WinUI Compatibility Preview 開放 Expert 的每-App 路由規則完整工作流程——有界清單（前 6 筆摘要＋溢出計數）、可達的移除按鈕、ID／App ID／顯示名稱、Lane、output group、priority、makeup gain、enabled 與 gain owner，提供新增／更新、逐筆移除與全部清除；重用既有 ViewModel 驗證（64 筆上限）、優先級排序、保存回復、狀態文字與本機 JSON 持久化，控制命令只在引擎已同步且 Ack 後才標示為已套用，實體 per-App 重新送出仍未驗證（Issue #804 / PR #808）。SPEC-0003 補上 BasicNoiseSuppressorV1 與 VirtualMicDspV1 共用的 upper-only 2 dB reopen hysteresis 契約：threshold 關閉、回升 +2 dB 才開啟，中間維持原狀態以避免臨界 chatter（Issue #810 / PR #812）。自訂 Scene catalog 新增 control-plane 同步：SceneCatalogCommandV1（IPC type 20）以固定 3260-byte wire format 送出 Upsert／Remove／Clear，engine control worker 擁有 mutable catalog 並透過既有 `SceneCatalogV1::upsert` 在驗證後原子替換 definition；C# 控制模型在連線時於新增或刪除本機卡片後推送同步；契約測試涵蓋 encode→decode round-trip、engine apply、scene apply resolution、remove 與 corrupted-payload rejection（Issue #768 / PR #814）。Wave34 全域快照 evidence 時間戳修正為實際 gate 完成時間與完整命令紀錄，並明確記錄 post-merge correction（Issue #809 / PR #811）。上述分別是 compatibility preview source/build/launch、documentation/spec、source+contract test/control-model/engine smoke 或 documentation/evidence correction；不宣稱 runtime screen-reader audit、實體音訊量測、driver guest 安裝／載入／HLK 或 Microsoft signing。
