...
第三十九波整合增量已合併：calibration-response schema 的 `measured_db` 與
`target_db` 加入 -144..12 dB 明確邊界，與既有系統 volume floor／safety ceiling
慣例一致，讓不合理校準數值在 JSON contract 層即被拒絕；SPEC-0002 同步記錄此契約。
此為 schema/spec/evidence 增量，runtime compiler 行為不變（Issue #957 / PR #960）。
屬 documentation/contract evidence；不宣稱實體音訊量測、driver 安裝／載入／HLK 或
Microsoft signing。
...
目前驗證摘要：`verify.ps1` 的 3 個 CTest（contract_tests、asio_transport_selftest、tab_bridge_selftest）通過；`docs-check.ps1` 的 86 個必要入口與
