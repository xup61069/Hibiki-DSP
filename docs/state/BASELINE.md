...
第五十七波（wave 57）整合增量已合併一項能力：校正精靈的有界 PEQ 編譯器新增專屬行為測試執行檔 hibiki_calibration_compiler_tests，覆蓋目標曲線（Flat / Harman 入耳 / Harman 罩耳）名稱與取值邊界 fail-closed、編譯政策與量測資料驗證拒收空資料／未排序／重複頻率／超過 dB 上限／NaN、平坦響應不產生 filter、貪婪選擇最大殘差、ignore threshold 內誤差不選點、boost/cut 超上限夾限並標記 limited、最小 filter 間距與 max_filters 硬上限，以及 Q 值政策視窗契約，讓校正精靈的核心編譯路徑從無專屬單元覆蓋升級為獨立可驗證的行為契約；全部使用合成頻率響應資料，不含 ISO 226 授權表格或真實量測（Issue #1795 / PR #1799）。以上皆屬 user-space source/control/docs evidence；不宣稱房間最佳化、實體校正、microphone、driver/WaveRT 或實體音訊。
