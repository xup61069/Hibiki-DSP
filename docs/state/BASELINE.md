...
第三十八波（wave 38）整合增量已合併五项能力与修正: (1) WinUI DSP graph node cards unified to SectionCardStyle (Issue #1455 / PR #1457). (2) Engine enables live loudness recompute by default on built-in Game/Movie/Voice scenes (Issue #1445 / PR #1458). (3) WinUI custom preset rows unified to shared card style (Issue #1459 / PR #1460). (4) Engine fixes scene live-update opt-in commit order so the enable flag survives the final loudness PEQ commit, with a direct update_loudness_phon regression assertion (Issue #1463 / PR #1464). (5) Engine Preview wires BasicNoiseSuppressorV1 into the tab bridge lane as an explicit --enable-tab-noise-suppressor opt-in with fail-closed setup and route-health detail; this is a bounded high-pass plus downward-gate connection, not AI denoising (Issue #1456 / PR #1461). All are user-space source/UI/engine-contract/docs evidence; no driver/WaveRT/HLK or signing claims.
...
目前驗證摘要：`verify.ps1` 的 5 個 CTest（hibiki_contract_tests、hibiki_driver_stream_tests、hibiki_noise_suppressor_tests、hibiki_asio_transport_selftest、hibiki_tab_bridge_selftest）通過（含 #1474 新增的 suppressor fail-closed 測試）；`docs-check.ps1` 的 87 個必要入口與
