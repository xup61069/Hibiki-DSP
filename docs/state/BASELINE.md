...
與 VST3 latency graph commit／RT lane latency bank／parameter timeline／Scene automation refs／rollback lifecycle／UI device fade／plugin lane token baseline `c621d4b`；
新 AI 接手時仍必須確認
working tree 與該 scope 是否一致。

目前驗證摘要：`verify.ps1` 的 1 個 CTest 通過；`docs-check.ps1` 的 54 個必要入口與
12 份 Spec 通過；`source-policy.ps1` 掃描 236 個路徑且無 blocked binary/secret；
