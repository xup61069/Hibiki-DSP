...
第四十一波（wave 41）整合增量已合併兩項能力：(1) Engine Preview WAV file source 新增取樣率自動轉換：當 --wav-source-path 指定的檔案取樣率與 sink 不一致時，以 bounded polyphase filter 在離線解碼後、送入 user-space graph 前完成轉換，讓 44.1 kHz 檔案能在 48 kHz sink 上正確播放而不改變 pitch；此為本機檔案播放的 user-space graph evidence，不是 endpoint policy 或實體 driver 驗收（Issue #1517 / PR #1519）。(2) Control model 與 Engine 新增自適應程式感知 EQ 視覺化幀（source=2）：Engine 在每次 process 後把 bounded slow-level proxy 的已套用增益存入 lock-free telemetry snapshot，control plane 以 debounce + rate limit 發布為 4 點低頻 shelf 曲線，讓 EQ 曲線隨內容音量校正即時更新；stale sequence 以獨立 domain 判定避免與 source=1 干擾。此為 control-plane visual evidence，不是 BS.1770/ISO conformance 或實體音訊 evidence（Issue #1515 / PR #1522）。以上皆屬 user-space source/engine contract/control/docs evidence；不宣稱 driver/WaveRT/HLK 或簽章。
