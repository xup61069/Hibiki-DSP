...
docs/evidence 增量。
目前驗證摘要：`verify.ps1` 的 26 個 CTest（hibiki_contract_tests、hibiki_driver_stream_tests、hibiki_equal_loudness_tests、hibiki_ir_convolver_tests、hibiki_lane_latency_tests、hibiki_noise_suppressor_tests、hibiki_output_crossfade_tests、hibiki_peq_dsp_tests、hibiki_program_loudness_tests、hibiki_volume_state_tests、hibiki_wav_ir_tests、hibiki_session_route_rule_tests、hibiki_asio_transport_selftest、hibiki_tab_bridge_selftest、hibiki_output_fanout_tests、hibiki_output_sink_tests、hibiki_session_command_queue_tests、hibiki_virtual_mic_tests、hibiki_calibration_compiler_tests、hibiki_asio_transport_consumer_tests、hibiki_true_peak_limiter_tests、hibiki_device_recovery_tests、hibiki_scene_safety_tests、hibiki_ir_phase_tests、hibiki_device_switch_tests、hibiki_process_loopback_plan_tests）通過（含 #1474 新增的 suppressor fail-closed 測試、#1647 新增的 lane latency 行為測試、#1667 新增的 output crossfade 行為測試、#1660 新增的 PEQ DSP 行為測試、#1677 新增的 program loudness 行為測試、#1684 新增的 true peak limiter 行為測試、本切片新增的 output fanout plan validation 與 interleaved copy 行為測試、Issue #1715 新增的 IR convolver 邊界行為測試、device recovery coordinator 狀態機行為測試、Issue #1725 新增的 volume state ramp 與 bank boundary 行為測試、#1730 新增的 WAV IR decoder 與 convolver prepare 行為測試、scene safety controller 音量安全行為測試（Issue #1754）、session route rule store 優先序與歧義 fail-closed 行為測試（Issue #1765）、IR phase policy 驗證與解析行為測試（Issue #1764）、output sink ring buffer 環繞/溢位、clock drift 夾限、linear 與 persistent polyphase resampler 邊界與收斂行為測試（Issue #1772）、device switch transaction 生命週期與 fail-closed 邊界行為測試（Issue #1774）、virtual mic NLMS 收斂方向、noise gate 開合遲滯與 privacy mute fail-closed 行為測試（Issue #1779）、process loopback plan 容量上限與過濾語意行為測試（Issue #1777）、calibration target curve 取值邊界與 bounded PEQ 編譯增益夾限行為測試（Issue #1795），以及 session command queue SPSC 環繞、跨環繞 FIFO、payload 隔離與 reset 槽位清除行為測試（Issue #1800））；`docs-check.ps1` 的 86 個必要入口與
