...
- Bounded multi-sink output fan-out now rejects all-disabled plans and non-finite input blocks
  ...
  Master/limiter and the per-sink runtime without duplicating or restarting the graph.
...
與 VST3 latency graph commit／RT lane latency bank／parameter timeline／Scene automation refs／rollback lifecycle／UI device fade／plugin lane token／multi-sink fan-out、per-sink clock/SRC runtime、AudioEngine fan-out boundary 及其 fail-closed safety baseline `2ae1b80`；
新 AI 接手時仍必須確認
