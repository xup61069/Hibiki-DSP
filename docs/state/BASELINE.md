...
第三十五波（wave 35）整合增量已合併兩項 WinUI 拋光：(1) 新增全域隱含 ComboBox 樣式於 Styles/Input.xaml：CornerRadius=8、MinHeight=36，與先前 TextBox 樣式對齊，經 App.xaml 資源字典合併套用到全部 5 個 ComboBox 實例，不需逐一修改實例標記（Issue #1404 / PR #1405）。(2) Slider 軌道填充改用柔和 accent 色：6px 圓角軌道（corner radius 2），比全強度 accent 更輕量；section card 在指標 hover 時新增 accent 邊框高光；同時移除 PR #1383 留下的 dead ModernSliderStyle 與 dead HibikiSliderTrackFillBrush 主題資源，樣式集中到新的 Styles/ControlPolish.xaml 字典（Issue #1393 / PR #1397）。以上皆為 user-space UI/build/docs evidence；不宣稱 driver/WaveRT/HLK 或簽章能力。
