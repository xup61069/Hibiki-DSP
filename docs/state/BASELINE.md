...
目前驗證摘要：`verify.ps1` 的 43 個 CTest（hibiki_contract_tests、hibiki_driver_stream_tests、hibiki_equal_loudness_tests、hibiki_ir_convolver_tests、hibiki_lane_latency_tests、hibiki_noise_suppressor_tests、hibiki_output_crossfade_tests、hibiki_control_status_tests、hibiki_peq_dsp_tests、hibiki_program_loudness_tests、hibiki_volume_state_tests、hibiki_wav_ir_tests、hibiki_session_route_rule_tests、hibiki_asio_transport_selftest、hibiki_tab_bridge_selftest、hibiki_output_fanout_tests、hibiki_output_sink_tests、hibiki_session_command_queue_tests、hibiki_virtual_mic_tests、hibiki_calibration_compiler_tests、hibiki_asio_transport_consumer_tests、hibiki_true_peak_limiter_tests、hibiki_device_recovery_tests、hibiki_scene_safety_tests、hibiki_exporters_tests、hibiki_ir_phase_tests、hibiki_scene_presets_tests、hibiki_driver_volume_link_tests、hibiki_output_handoff_tests、hibiki_device_catalog_snapshot_tests、hibiki_calibration_anchor_tests、hibiki_vst3_sandbox_launch_tests、hibiki_windows_volume_callback_tests、hibiki_ipc_codec_tests、hibiki_ipc_pipe_tests、hibiki_session_catalog_snapshot_tests、hibiki_scene_graph_tests、hibiki_audio_session_registry_tests、hibiki_vst3_lane_bridge_tests、hibiki_device_switch_tests、hibiki_ir_phase_kernel_tests、hibiki_process_loopback_plan_tests、hibiki_windows_device_watcher_tests）通過（含 control status snapshot 往返、fail-closed 驗證與 store 行為測試（Issue #1813）、#1474 新增的 suppressor fail-closed 測試、#1647 新增的 lane latency 行為測試、#1667 新增的 output crossfade 行為測試、#1660 新增的 PEQ DSP 行為測試、#1677 新增的 program loudness 行為測試、#1684 新增的 true peak limiter 行為測試、本切片新增的 output fanout plan validation 與 interleaved copy 行為測試、Issue #1715 新增的 IR convolver 邊界行為測試、device recovery coordinator 狀態機行為測試、Issue #1725 新增的 volume state ramp 與 bank boundary 行為測試、#1730 新增的 WAV IR decoder 與 convolver prepare 行為測試、scene safety controller 音量安全行為測試（Issue #1754）、session route rule store 優先序與歧義 fail-closed 行為測試（Issue #1765）、IR phase policy 驗證與解析行為測試（Issue #1764）、output sink ring buffer 環繞/溢位、clock drift 夾限、linear 與 persistent polyphase resampler 邊界與收斂行為測試（Issue #1772）、device switch transaction 生命週期與 fail-closed 邊界行為測試（Issue #1774）、virtual mic NLMS 收斂方向、noise gate 開合遲滯與 privacy mute fail-closed 行為測試（Issue #1779）、process loopback plan 容量上限與過濾語意行為測試（Issue #1777）、calibration target curve 取值邊界與 bounded PEQ 編譯增益夾限行為測試（Issue #1795），以及 session command queue SPSC 環繞、跨環繞 FIFO、payload 隔離與 reset 槽位清除行為測試（Issue #1800）、IR phase kernel 邊界行為測試（Issue #1705）、PEQ exporter 格式與 fail-closed 行為測試（Issue #1774），以及 easy scene presets 共用預設與 Game/Movie/Voice/Studio 行為測試（Issue #1806），以及 windows device watcher COM 回呼 bounded copy、null id 容錯、poll 序號語意與 ref counting 行為測試（Issue #1812），以及裝置快照 publisher 容量邊界、descriptor 映射、wire padding、store 序列與 reply 幀行為測試（Issue #1814）、#1757新增的 output handoff coordinator 行為測試，以及 driver 音量連結封包往返、apply 拒收分支、世代語意與忽略清單行為測試（Issue #1836）、session catalog snapshot wire codec 與 store 序列、容量邊界與 reply 幀 fail-closed 行為測試（Issue #1842）、named pipe 控制通道設定驗證、malformed framing 拒收、handler Error 回應與 idle timeout 復原行為測試（Issue #1849）、IPC codec 錯誤路徑 decode 拒收與 fail-closed 行為測試（Issue #1850）、calibration acoustic anchor 欄位邊界與 phon 估算 fail-closed 行為測試（Issue #1858）、Windows 音量 COM 回呼快照複製、通道夾限與未綁定 broker fail-closed 行為測試（Issue #1860）、VST3 沙盒啟動驗證 watchdog/pipe/sample rate/channel 邊界與 passthrough 語意行為測試（Issue #1880））；`docs-check.ps1` 的 86 個必要入口與
