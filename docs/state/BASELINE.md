...
第六十二波（wave 62）整合增量已合併一項能力與修正：(1) hosted verify.yml 新增 Control model scene sync failure gate step，執行 tools/control-model-scene-sync-check.ps1：該檢查由 commit 71158039（fix(control-model): persist scene sync failures，Fixes #2019）引入，涵蓋連線中引擎 Error 回覆時 add/remove 場景操作的 failure 紀錄跨重啟持久化、以及 queue 無法存檔時 card、selection 與 card file 的 rollback 還原，但從未被任何 workflow、docs 或 evidence 引用（workflow 工具引用盤點中 46 個 tools/*.ps1 有 29 個未被引用，此為其中唯一守護已合併修正的離線 contract check）；hosted CI 原有 #832 Case 3 NACK 場景早於 #2019 修正，不涵蓋上述兩項行為，本波補上後 regression 將被 hosted gate 攔截。本波屬 CI gate 與 docs/evidence 增量；檢查本身為 user-space control-model evidence，不宣稱 physical audio、driver、WaveRT、簽章、installer、accessibility 或 release readiness（Issue #2261）。
第六十一波（wave 61）整合增量已合併一項能力與修正：(1) hosted verify.yml 的 Gate self-test sweep 門檻從 26 提升至 43：該 fail-closed 下限在 foundation commit 425ba5b3（evidence/0000-foundation/sweep-threshold-26-v1.json）設定時 tools/*.ps1 恰有 26 個 -SelfTest 模式，其後工具集持續擴增至 43 個（hosted run 33587258357 於 main 429ef1d7 回報 42 modes，wave 60 加入 tools/verify-theme-fallbacks.ps1 後本地 Get-Command 探索為 43），過時下限代表最多 17 個 self-test 模式可因回歸、改名或誤刪參數而不被 CI 察覺；本波將 minimumModes 對齊目前 fail-closed 清單 43，任何單一 self-test 模式消失即觸發 sweep 失敗。本波屬 CI 門檻與 docs/evidence 增量；不宣稱 hosted runner 對各模式的語意覆蓋，亦不宣稱 driver、WaveRT、簽章、installer、accessibility 或 release evidence（Issue #2259）。
第六十波（wave 60）整合增量已合併一項能力與修正：(1) theme fallback byte-for-byte 驗證從本機 pinned package 環境提升為 hosted CI gate：新增 versioned `tools/verify-theme-fallbacks.ps1`，預設從 nuget.org flat container 下載 pinned microsoft.windowsappsdk 1.7.250401001 nupkg（SHA-512 fail-closed 驗證）、解出 lib/net6.0-windows10.0.22621.0/Microsoft.WinUI/Themes/generic.xaml（SHA256 0A32014884636EFE31FE7D71F6C620705E3D5FC3A90ADD1EE93E03F478912408 fail-closed 驗證）後呼叫 extract-theme-fallbacks.ps1 -VerifyCommitted -XamlPath，`-PackagePath` 可改驗既有 nupkg，暫存只寫入自有 temp 目錄並清理；verify.yml 新增 hosted step 在每個 PR verify 執行此驗證，`-SelfTest` 以 in-memory zip fixture 覆蓋 pinned 常數形狀、csproj 版本耦合、URL 建構、entry 選擇與 streaming hash、缺 entry fail-closed 與 extractor wiring，不連網、不寫檔；SPEC-0010 provenance 與 PROJECT_MAP tools 列同步（Issue #2257 / PR #2258）。本波屬 tools/CI/docs evidence；不宣稱 driver、WaveRT、簽章、installer 或 release readiness，亦不宣稱 hosted gate 等於正式 XAML、accessibility 或 release evidence。
第五十九波（wave 59）整合增量已合併一項能力與修正：(1) theme fallback extractor 從機器本機 `.local/` 遷移為 versioned `tools/extract-theme-fallbacks.ps1`：從 pinned microsoft.windowsappsdk 1.7.250401001 的 generic.xaml（SHA256 0A32014884636EFE31FE7D71F6C620705E3D5FC3A90ADD1EE93E03F478912408）解碼重建 App.xaml.cs 的 committed typed theme fallback 區塊，`-VerifyCommitted` 在本機對 3242 rows／222459 bytes 達成 byte-for-byte 相符且 committed 字典不變，`-SelfTest` 以合成 XAML fixture 覆蓋 extraction rules（Light-first first-wins、root fallback、StaticResource alias 鏈 elevation/acrylic 近似降級、hex 正規化、Thickness/CornerRadius quirks、skip 類型）與 committed 結構 invariants（16 空格 marker、12 空格 row 格式、唯一 keys、token prefix 白名單、base 與 approx 兩段 invariant culture 忽略大小寫排序、approx token 邊界）；SPEC-0010 provenance 同步改指 versioned tool（Issue #2255 / PR #2256）。本波屬 source/tools/docs evidence；不宣稱 driver、WaveRT、簽章、installer 或 release readiness，byte-for-byte 驗證綁定本機 pinned package 環境。
第五十八波（wave 58）整合增量已合併四項能力與修正：(1) 公開 v1.0.0 portable user-space preview Release 完成發布 readback 證據：GitHub Release 非 draft、非 prerelease，兩個 assets 的全新下載 SHA-256、sidecar、fresh extraction、manifest、466 個 payload hash 與 hidden launch smoke 通過，來源綁定已合併的 a172def0 snapshot（Issue #2245 / PR #2246）。(2) build-preview.ps1 把 Get-PreviewAutomationCount 從 SelfTest 區塊移到 script scope，讓 -Target WinUI -SmokeTest 在正常建置成功後可執行 smoke 而不再誤報 helper 不存在；SelfTest 行為不變（Issue #2247 / PR #2248）。(3) Compatibility Preview 修復啟動 race 造成 XamlParseException / stowed exception 0xC000027B 的問題：OnLaunched 先把 3242 個 typed theme fallback keys 一次合併到 Application.Resources，提供 Color、Thickness、CornerRadius、Double、FontWeight、FontSize、brush strings 與 Style target types 的降級資源面；fallback 值來自機器本機 extractor，包含已記錄的 elevation 與 acrylic 近似，缺漏資源呈 fallback-styled 而非 unstyled 外觀。本機 compat smoke 10/10 啟動 EXIT=0，keys=3242、failed=0（Issue #2249 / PR #2250）。(4) SPEC-0010 相容性段落同步上述 typed fallback dictionary 行為、鍵型別、來源與近似邊界，並修正為 fallback-styled 降級外觀（Issue #2251 / PR #2252）。portable readback 不改 source，也不宣稱簽章、HLK、installer、升級、回滾、解除安裝、實體音訊、driver 或 WaveRT delivery；compat smoke 不等同正式 XAML、accessibility 或 release evidence；fallback extractor 仍留在機器本機 .local/，本波未遷移到 tools/。
第五十七波（wave 57）整合增量已合併三項能力與修正：(1) SessionCommandQueueRing 新增專屬行為測試執行檔 hibiki_session_command_queue_tests，覆蓋 SPSC 環繞、跨環繞 FIFO 順序、payload 隔離、64-slot 滿載 fail-closed、drop counter 與 reset 槽位清除，讓 session command queue 的 lock-free 環形緩衝從片段覆蓋升級為獨立可驗證契約，CTest 從 24 個擴充到 25 個 target（Issue #1800 / PR #1803）。(2) 校正精靈的有界 PEQ 編譯器新增專屬行為測試執行檔 hibiki_calibration_compiler_tests，覆蓋目標曲線（Flat / Harman 入耳 / Harman 罩耳）取值邊界、編譯政策與量測資料驗證拒收空資料／未排序／重複頻率／超過 dB 上限／NaN、貪婪選擇最大殘差、boost/cut 夾限與 limited 標記、最小 filter 間距與 max_filters 硬上限，以及 Q 值政策視窗；全部使用合成頻率響應資料，不含 ISO 226 授權表格或真實量測，CTest 擴充到 26 個 target（Issue #1795 / PR #1799）。(3) 正式 WinUI shell 收緊共用 ToggleSwitch 的 header template，讓標題列主題切換在標準視窗尺寸下完整顯示「深色」標籤與開關狀態，不再擠壓到邊框底部；Expert 與靜音等表單 toggle 在淺色與深色模式維持可讀性（Issue #1805 / PR #1809）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱房間最佳化、實體校正、microphone、driver/WaveRT 或實體音訊。
第五十六波（wave 56）整合增量已合併兩項能力與修正：(1) build_process_loopback_plan 新增專屬行為測試執行檔 hibiki_process_loopback_plan_tests，覆蓋同 process 多 session 聚合、inactive/缺 lane/缺 output_group 過濾語意、fail-closed 重置與恰好 64 個 process 容量邊界，讓 process loopback 計畫編譯器從片段覆蓋升級為獨立可驗證契約（Issue #1802 / PR #1787）。(2) 正式 WinUI shell 的 NumberBox 表單欄位新增隱含樣式，圓角與最小高度對齊既有 TextBox/ComboBox，讓 route 與校準頁的數字輸入欄位視覺一致並保留平台 spin-button 行為（Issue #1789 / PR #1790）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第五十五波（wave 55）整合增量已合併一項能力：VirtualMicDspV1 與 VirtualMicRouteModel 新增專屬行為測試執行檔 hibiki_virtual_mic_tests，覆蓋 prepare 參數邊界 fail-closed、NLMS 收斂方向、noise gate 開合遲滯、null/零幀/NaN 拒絕、隱私靜音預設與 echo reference 雙側啟用契約，CTest 從 21 個擴充到 23 個 target（Issue #1797 / PR #1788）。以上皆屬 user-space source/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第五十四波（wave 54）整合增量已合併一項能力修正：正式 WinUI shell 恢復淺色主題的三層表面層級，殼層底色、section card 與 scene/route card 各有明確視覺分離，使用者能輕鬆掃描頁面內容而不會覺得所有元素融為一片近白平面；深色模式維持既有分層處理（Issue #1792 / PR #1784）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第五十三波（wave 53）整合增量已合併三項能力與修正：(1) DeviceSwitchTransaction 新增專屬行為測試執行檔 hibiki_device_switch_tests，覆蓋完整生命週期、亂序 prepare/commit 拒絕、rebind rollback 還原與 degraded 狀態 fail-closed 處理，讓裝置切換的音訊保護狀態機從無專屬覆蓋升級為獨立可驗證契約，CTest 從 21 個擴充到 22 個 target（Issue #1785 / PR #1780）。(2) output_sink 模組新增專屬行為測試執行檔 hibiki_output_sink_tests，覆蓋 InterleavedRingBuffer 環繞/溢位、ClockDriftEstimator 夾限與 PersistentPolyphaseResampler prepare/process 契約，CTest 從 20 個擴充到 21 個 target（Issue #1772 / PR #1775）。(3) 正式 WinUI shell 讓緊湊狀態徽章依內容自適應寬度而非被壓縮到不可讀，改善深色與淺色主題下的資訊可見性（Issue #1768 / PR #1770）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第五十二波（wave 52）整合增量已合併三項能力與修正：(1) SessionRouteRuleStoreV1 新增專屬行為測試執行檔 hibiki_session_route_rule_tests，覆蓋 upsert 驗證、大小寫不敏感匹配規則、優先權衝突 fail-closed 處理與 64-rule 容量邊界，讓 per-App 音訊路由規則從分散的 wire-encoding 測試升級為獨立可驗證契約（Issue #1778 / PR #1767）。(2) IrPhasePolicyV1 新增專屬單元測試執行檔 hibiki_ir_phase_tests，覆蓋全部 phase mode 的驗證邊界、strength 範圍與 delay/FIR 計算契約，CTest 擴充到 19 個 target，為 IR phase 政策建立可驗證行為（Issue #1778 / PR #1766）。(3) 瀏覽器擴充功能診斷新增瀏覽器擷取持續時間與最近音訊封包年齡顯示，讓使用者直接看出分頁音訊仍在播放且資料持續流動（Issue #1778 / PR #1751）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第五十一波（wave 51）整合增量已合併三項能力與修正：(1) 正式 WinUI shell 在切換主導覽頁面時把共用內容 ScrollViewer 的垂直位置重設回頂端，使用者從長頁面捲到中段後切換頁面不再遺失新頁標題與第一組控制（Issue #1734 / PR #1758）。(2) 修復 live probe 的 Float32 格式拒收、multi-soak 空格路徑引號問題，以及 docs-check 在 nested clone 中的 CTest 發現，讓診斷工具在更多環境下可靠運作（Issue #1724 / PR #1726）。(3) 正式 WinUI 讓 SceneCard 鍵盤焦點視覺清晰可辨，鍵盤使用者能明確看出目前聚焦的場景卡片，改善無障礙導航（Issue #1749 / PR #1760）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第五十波（wave 50）整合增量已合併三項能力與修正：(1) 正式 WinUI shell 將深色模式的主要殼層表面與 Mica 純黑分離，重新定義深色主題的表面層級，讓主要內容不再與背景融為一片黑（Issue #1748 / PR #1752）。(2) 音量狀態控制模組新增專屬行為測試執行檔，覆蓋 volume state ramp 與 bank boundary 行為，為 program-aware 音量控制補齊可驗證契約（Issue #1725 / PR #1733）。(3) 正式 WinUI 自訂預設區新增匯出／匯入按鈕，重用既有 ViewModel 的 fail-closed 匯入流程：無效、空、超容量檔案不改變現有卡片，成功後刷新清單並如實回報；與 DesktopCompat 預覽達成 JSON 往返一致（Issue #1721 / PR #1747）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第四十九波（wave 49）整合增量已合併三項能力與修正：(1) DeviceRecoveryCoordinator 新增專屬行為測試執行檔，覆蓋事件驅動狀態機、交易式 rebind 流程與安全重啟音量鉗制邊界，讓裝置恢復協調器從分散在大型 contract tests 中的片段覆蓋升級為獨立可驗證契約（Issue #1719 / PR #1731）。(2) 正式 WinUI shell 改善深色主題表面對比：section card、route/item card 與文字層級重新分層，主要內容不再像整片黑底，同時保留深色模式與淺色模式行為（Issue #1739 / PR #1742）。(3) wav_ir 解碼層（decode_ir_wav_v1 / prepare_ir_convolver_from_wav_v1）新增專屬測試執行檔 hibiki_wav_ir_tests，涵蓋 PCM16/24/32 縮放、chunk padding、截斷 header、重複或缺少 fmt/data chunk、聲道／取樣率／block align 拒收、零幀資料、prepare 驗證與 mono broadcast 等 fail-closed 路徑；CTest 擴充到 14 個 target（Issue #1730 / PR #1743）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第四十八波（wave 48）整合增量已合併三項能力與修正：(1) Engine 修復 ProgramAwareLevelBankV1 的兩個記憶體安全缺陷：64-byte label 寫入越界（OOB write）與 reset_all 的 group_count 不一致，讓 program-aware 音量控制在邊界輸入下不再破壞相鄰記憶體，並補上回歸測試覆蓋越界路徑與重置一致性（Issue #1698 / PR #1700）。(2) 正式 WinUI shell 將 Expert 路由頁的 route health、App session 與 route rule 卡片統一為獨立卡片樣式，多筆資料渲染時卡片間保留一致垂直間距，不再出現邊框與圓角貼在一起的視覺擁擠問題（Issue #1722 / PR #1727）。(3) 即時音訊摺積引擎 IrConvolverV1 新增專屬單元測試執行檔 hibiki_ir_convolver_tests 與 CTest target，覆蓋 prepare 邊界、null buffer、零幀、non-finite 參數等 fail-closed 路徑，為核心 DSP 模組建立可驗證的行為契約（Issue #1715 / PR #1720）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第四十七波（wave 47）整合增量已合併八項能力與修正：(1) DesktopCompat 備用 UI 的 route rule 面板新增匯出與匯入按鈕，可將 route rule catalog 保存為 JSON 檔並在其他機器載回；匯入先以 bounded schema 驗證整個檔案再 upsert 到現行 catalog，save 失敗會 rollback 新增的 rules（Issue #1682 / PR #1685）。(2) 正式 WinUI shell 恢復校準頁面的導航圖示，並記錄非空 navigation icon 契約（Issue #1691 / PR #1696）。(3) TruePeakLimiterV1 新增獨立測試執行檔，覆蓋 null buffer、零幀、0 或 >8 聲道、non-finite ceiling 與超範圍取樣率的 fail-closed；ceiling 鉗制在 [-144, 0] dBTP；低於 ceiling 的訊號逐位通過（Issue #1684 / PR #1693）。(4) 等響度補償模組新增獨立測試執行檔 hibiki_equal_loudness_tests，覆蓋 validate_policy 全部邊界、phon domain 拒絕、1 kHz anchor 正規化、strength 線性縮放與 boost cap（Issue #1694 / PR #1702）。(5) WinUI Easy 模式的 Expert 面板新增有用的空狀態說明與直接啟用 Expert view 的路徑，讓使用者知道如何開啟進階控制（Issue #1701 / PR #1704）。(6) WinUI shell Spec 記錄校準頁面與 Ctrl+7 導航快速鍵（Issue #1708 / PR #1709）。(7) DesktopCompat 自訂場景面板新增匯出與匯入按鈕，使用既有 bounded schema 的原子寫入路徑，匯入預先檢查容量避免部分寫入（Issue #1692 / PR #1697）。(8) Output fanout 模組新增獨立測試執行檔 hibiki_output_fanout_tests，覆蓋 plan 驗證（schema、revision、channel count、sink bounds、ID 重複與 non-printable）、prepare 邊界與 interleaved copy 行為（Issue #1712 / PR #1713）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
第四十六波（wave 46）整合增量已合併十項能力與修正：(1) DesktopCompat 預覽殼層新增視窗大小與位置記憶，拖曳或縮放後下次啟動自動還原；bounds 離屏或過小時回退預設置中（Issue #1654 / PR #1655）。(2) 正式 WinUI shell 新增深色模式與標題列明暗切換按鈕，主題偏好保存到 %LOCALAPPDATA%；音量保護頁面的卡片提升為「等響度補償」並加入誠實的離線狀態與非色彩圖例（Issue #1650）。(3) Engine 的 OutputCrossfade sink 轉場新增獨立行為測試執行檔，涵蓋不支援聲道數、取樣率、時長的 fail-closed，以及合法邊界配置的正確幀數計算與 null 指標拒絕（Issue #1667 / PR #1672）。(4) Control model 的聆聽劑量指示器新增靜音暫停提示文字，muted 期間顯示「靜音中：劑量暫停累積」且計費從解除靜音的新樣本重新起算（Issue #1662 / PR #1665）。(5) PEQ DSP processor 新增行為測試執行檔 hibiki_peq_dsp_tests，覆蓋參數驗證、增益恆等、RBJ 穩態設計值、級聯、多聲道獨立性與 fail-closed 路徑；CTest 從 6 個擴充到 7 個（Issue #1660 / PR #1661）。(6) BasicNoiseSuppressorV1 新增目標行為測試，覆蓋安靜輸入 gate 收斂至 floor gain、持續大聲輸入 gate 收斂至 unity gain 及中間狀態行為（Issue #1636 / PR #1669）。(7) WinUI 校準精靈支援多聲道測量檔案批次匯入（.csv / .txt），每聲道獨立記錄 WizardChannelMeasurement，UI 提供可及性摘要與編譯前預覽（PR #1671）。(8) DesktopCompat 預覽殼層新增輸出群組選擇記憶（main / low-latency / surround），以 fail-closed ID 規則驗證並存入既有 UI 狀態檔（Issue #1673 / PR #1675）。(9) AsioTransportConsumerV1 新增獨立測試執行檔 hibiki_asio_transport_consumer_tests，覆蓋 bind/unbind 生命週期、格式不符拒絕、pop fail-closed 路徑與 dropped_blocks 回讀（Issue #1663 / PR #1668）。(10) SPEC-0002 一致性免責聲明移除殘留授權標準名稱，改為等響度措辭（Issue #1681 / PR #1680）。以上皆屬 user-space source/UI/control/docs evidence；不宣稱 driver/WaveRT 或實體音訊。
...
目前驗證摘要：`verify.ps1` 的 52 個 CTest（hibiki_contract_tests、hibiki_driver_stream_tests、hibiki_equal_loudness_tests、hibiki_ir_convolver_tests、hibiki_lane_latency_tests、hibiki_noise_suppressor_tests、hibiki_output_crossfade_tests、hibiki_control_status_tests、hibiki_peq_dsp_tests、hibiki_program_loudness_tests、hibiki_volume_state_tests、hibiki_wav_ir_tests、hibiki_session_route_rule_tests、hibiki_asio_transport_selftest、hibiki_tab_bridge_selftest、hibiki_output_fanout_tests、hibiki_output_sink_tests、hibiki_session_command_queue_tests、hibiki_virtual_mic_tests、hibiki_calibration_compiler_tests、hibiki_asio_transport_consumer_tests、hibiki_true_peak_limiter_tests、hibiki_device_recovery_tests、hibiki_scene_safety_tests、hibiki_exporters_tests、hibiki_ir_phase_tests、hibiki_scene_presets_tests、hibiki_driver_volume_link_tests、hibiki_output_handoff_tests、hibiki_device_catalog_snapshot_tests、hibiki_calibration_anchor_tests、hibiki_windows_volume_callback_tests、hibiki_ipc_codec_tests、hibiki_ipc_pipe_tests、hibiki_session_catalog_snapshot_tests、hibiki_scene_graph_tests、hibiki_audio_session_registry_tests、hibiki_vst3_lane_bridge_tests、hibiki_device_switch_tests、hibiki_ir_phase_kernel_tests、hibiki_process_loopback_plan_tests、hibiki_windows_device_watcher_tests、hibiki_control_plane_service_tests、hibiki_control_plane_tests、hibiki_scene_catalog_id_tests、hibiki_scene_catalog_tests、hibiki_vst3_sandbox_launch_tests、hibiki_session_route_graph_tests、hibiki_engine_control_tests、hibiki_control_volume_dispatch_tests、hibiki_control_payload_wire_tests、hibiki_ws_transport_tests）通過（含 control status snapshot 往返、fail-closed 驗證與 store 行為測試（Issue #1813）、#1474 新增的 suppressor fail-closed 測試、#1647 新增的 lane latency 行為測試、#1667 新增的 output crossfade 行為測試、#1660 新增的 PEQ DSP 行為測試、#1677 新增的 program loudness 行為測試、#1684 新增的 true peak limiter 行為測試、本切片新增的 output fanout plan validation 與 interleaved copy 行為測試、Issue #1715 新增的 IR convolver 邊界行為測試、device recovery coordinator 狀態機行為測試、Issue #1725 新增的 volume state ramp 與 bank boundary 行為測試、#1730 新增的 WAV IR decoder 與 convolver prepare 行為測試、scene safety controller 音量安全行為測試（Issue #1754）、session route rule store 優先序與歧義 fail-closed 行為測試（Issue #1765）、IR phase policy 驗證與解析行為測試（Issue #1764）、output sink ring buffer 環繞/溢位、clock drift 夾限、linear 與 persistent polyphase resampler 邊界與收斂行為測試（Issue #1772）、device switch transaction 生命週期與 fail-closed 邊界行為測試（Issue #1774）、virtual mic NLMS 收斂方向、noise gate 開合遲滯與 privacy mute fail-closed 行為測試（Issue #1779）、process loopback plan 容量上限與過濾語意行為測試（Issue #1777）、calibration target curve 取值邊界與 bounded PEQ 編譯增益夾限行為測試（Issue #1795），以及 session command queue SPSC 環繞、跨環繞 FIFO、payload 隔離與 reset 槽位清除行為測試（Issue #1800）、IR phase kernel 邊界行為測試（Issue #1705）、PEQ exporter 格式與 fail-closed 行為測試（Issue #1774），以及 easy scene presets 共用預設與 Game/Movie/Voice/Studio 行為測試（Issue #1806），以及 windows device watcher COM 回呼 bounded copy、null id 容錯、poll 序號語意與 ref counting 行為測試（Issue #1812），以及裝置快照 publisher 容量邊界、descriptor 映射、wire padding、store 序列與 reply 幀行為測試（Issue #1814）、#1757新增的 output handoff coordinator 行為測試，以及 driver 音量連結封包往返、apply 拒收分支、世代語意與忽略清單行為測試（Issue #1836）、session catalog snapshot wire codec 與 store 序列、容量邊界與 reply 幀 fail-closed 行為測試（Issue #1842）、named pipe 控制通道設定驗證、malformed framing 拒收、handler Error 回應與 idle timeout 復原行為測試（Issue #1849）、IPC codec 錯誤路徑 decode 拒收與 fail-closed 行為測試（Issue #1850）、control plane 命令路由 fail-closed、四種快照回覆綁定與命令佇列環繞行為測試（Issue #1861）、calibration acoustic anchor 欄位邊界與 phon 估算 fail-closed 行為測試（Issue #1858）、Windows 音量 COM 回呼快照複製、通道夾限與未綁定 broker fail-closed 行為測試（Issue #1860）、#1863新增的 control-plane routing lock 與 ring queue 行為測試、VST3 沙盒啟動驗證 watchdog/pipe/sample rate/channel 邊界與 passthrough 語意行為測試（Issue #1880）、engine control dispatch fail-closed 行為測試（Issue #1866）、control volume dispatch/queue fail-closed 行為測試（Issue #1885）、control payload wire boundary fail-closed 行為測試（Issue #1877）、WebSocket handshake maximum-size acceptance 與 one-byte-over rejection（Issue #2133））；`docs-check.ps1` 的 88 個必要入口與
26 份 Spec 通過；`source-policy.ps1` 掃描 tracked paths 且無 blocked
...
目前文件 gate 摘要：`docs-check.ps1` 的 91 個必要入口與
26 份 Spec 通過；`verify.ps1` 的 CTest 摘要維持上方的 52 個既有 target。
