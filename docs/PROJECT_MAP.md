...
| `vst-host/` | Out-of-process VST3 host | trusted/certified model + Job Object supervisor + tested frame codec/named-pipe boundary + bounded passthrough worker + optional pinned SDK factory catalog/one-main-bus processor/worker executable + bounded parameter frame + supervisor Hello/ProcessBlock exchange + fixed-capacity parameter timeline/lane session + PluginHostModel quarantine wiring + latency alignment/delay primitive + fixed-capacity latency graph commit transaction；Scene scheduler、back-pressure、UI automation editor、plugin state persistence、第三方 certification pending | SPEC-0008/0012 / plugin contract |
