...
| `src/` | C++20 RT graph、AudioEngine、per-output-group render、graph-to-WASAPI adapter、legacy channel-map＋optional 8×8 matrix routing、per-group plugin latency compensation 與跨 block lane delay bank、bounded multi-sink output fan-out、dB-domain volume ramp、session registry/session-route graph builder、Windows session adapter、Windows volume/device watcher/recovery、ISO math、program-aware RMS level controller、fixed-capacity PEQ biquad processor、bounded measured-response→PEQ calibration compiler、fixed-capacity FIR IR convolver、basic high-pass/downward-gate suppressor、IR phase policy、IPC codec/named-pipe worker、固定 SPSC control queue、Easy Scene control-worker adapter、transactional output handoff、persistent SRC、scene safety policy、sink crossfade、ASIO/外部 Lane block API、WASAPI Float32 output boundary、dual-worker WASAPI sink handoff、Windows WASAPI multi-sink fan-out、Virtual Mic privacy/reference、bounded echo cancellation/noise gate | 可測試 user-space；實體端點/driver 尚未 soak | SPEC-0001/0002/0003/0006/0007/0011/0012 |
...
| `vst-host/` | Out-of-process VST3 host | trusted/certified model + Job Object supervisor + tested frame codec/named-pipe boundary + bounded passthrough worker + optional pinned SDK factory catalog/one-main-bus processor/worker executable + bounded parameter frame + supervisor Hello/ProcessBlock exchange + fixed-capacity parameter timeline/lane session + bounded Scene automation store/scheduler with per-lane back-pressure + PluginHostModel quarantine wiring + latency alignment/delay primitive + fixed-capacity latency graph commit transaction；plugin state persistence、UI automation editor、side-chain/multi-bus、第三方 certification pending | SPEC-0008/0012 / plugin contract |
