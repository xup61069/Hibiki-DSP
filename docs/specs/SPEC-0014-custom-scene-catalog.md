...
佇列上限為 64 筆，超過時捨棄最舊操作、累計顯示已捨棄數量，且後續離線操作與重播
完成訊息不得覆蓋這個容量損失警告。控制管線重新連線成功後，ViewModel 依原順序把
佇列中的 Upsert／Remove 補送到引擎：全部成功才回報「引擎已同步」，同時保留先前
捨棄數量；中途失敗則保留剩餘操作並誠實顯示降級狀態，待下一次連線再補送。此重播
只使用既有 `SceneCatalogCommandV1` wire format 與 Ack 語意，不改變引擎端驗證或
catalog 容量契約；已連線時的同步同樣以收到引擎 Ack 為準，不得在送出前宣稱完成。
